...
def _sanitize_tool_args(name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name in ("search_kql", "aggregate"):
        ...
        if not _index_allowed(idx):
            if DEBUG:
                print(
                    f"[DEBUG][tools] index '{idx}' not in allow-list {ALLOW_INDICES}; using '{DEFAULT_INDEX}'",
                    flush=True,
                )
...
def summarize_findings(findings: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not API_KEY:
        ...
        if DEBUG:
            print("[DEBUG][openai tools] precheck failed ->", reason)
        return {
    ...
    with httpx.Client(timeout=90) as http:
        ...
        if r1.status_code >= 400:
            ...
            if DEBUG:
                print("[DEBUG][openai tools] first call failed ->", reason)
            return {
                "tldr": f"OpenAI tools error: {reason}. Rendering raw findings.",
                "severity": "Low",
                "evidence": findings[:10],
        ...
        if not tool_calls:
            ...
            if r_final.status_code >= 400:
                ...
                if DEBUG:
                    print("[DEBUG][openai tools] final call failed ->", reason)
                return {
                    "tldr": f"OpenAI tools error: {reason}. Rendering raw findings.",
                    "severity": "Low",
                    "evidence": findings[:10],
            ...
            try:
                ...
                if DEBUG:
                    print("[DEBUG][openai tools] json parse failed ->", reason, " RAW:", final_text[:400])
                return {
        ...
        for _ in range(2):
            ...
            for tc in tool_calls:
                ...
                try:
                    ...
                    if DEBUG:
                        print("[DEBUG][openai tools] tool arg parse error ->", reason)
                    return {
                        "tldr": f"OpenAI tools error: {reason}. Rendering raw findings.",
                        "severity": "Low",
                        "evidence": findings[:10],
        ...
        if DEBUG:
            print("[DEBUG][openai tools] loop limit ->", reason)
        return {
