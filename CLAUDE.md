...
1. **C# agent engine** (`src/TinySocs.Agent/Detection/`, rules in `packaging/detection/rules.yml`) — runs 39 rules of a single type (`threshold_by_key`) against raw Windows events in-process. **This is the only engine that actually fires alerts in production.**
...
When someone asks "how many rules does TinySOCs have," the honest answer is "39 actively detecting, 89 defined including the roadmap." Don't conflate.
