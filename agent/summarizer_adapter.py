...
def _salted_hash(blob: Any, salt: str) -> str:
    # use canonical JSON for stability; fall back to repr if needed
    try:
...
def _mask_any(v: Any) -> Any:
    ...
    if isinstance(v, dict):
        # BUGFIX: iterate key/value pairs (was iterating keys only)
...
def _extract_top_tokens(evidences: List[Dict[str, Any]]) -> List[str]:
    # very simple frequency of short “tokens” (exe names, event IDs, keywords)
        ...
        for k in ("summary", "exemplars"):
            ...
            for v in vals:
                ...
                for t in re.findall(r"[A-Za-z0-9._:-]{3,32}", text):
                    # de-prioritize pure numbers (noisy eventrecordids)
...
def _fingerprint_exemplar(ex: Any) -> str:
    # stable hash of exemplar with optional salt
...
def prepare_payload(evidences: List[Dict[str, Any]], window: str) -> Dict[str, Any]:
    ...
    - In 'abstract' mode: compact, masked fact table (no raw PII or full cmdlines).
    ...
    # produce masked/abstracted view
    minimal = []
    for e in evidences:
        rule = e.get("rule")
        ...
        # copy summary (masked) and normalize common strings
        ...
        # hashed exemplar fingerprints only (no raw exemplars)
    ...
    return {
        ...
        "aggregate": aggregate,
        "top_tokens": tokens,
        "notes": "Payload redacted: emails masked, IPs /24-coarsened, command lines truncated, exemplars hashed.",
        "minimal": minimal,   # safe to show to operators; still no raw exemplars
    }
...
def annotate_report_header(report_md: str, llm_mode: str = "openai") -> str:
    ...
    if report_md.startswith("#"):
        # place banner after the first H1
